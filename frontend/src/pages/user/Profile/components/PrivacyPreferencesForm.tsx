...
export const PrivacyPreferencesForm: React.FC<Props> = ({ userId, setDirty }) => {
  ...
  const ToggleItem = ({ label, desc, field }: { label: string, desc: string, field: keyof typeof prefs }) => (
    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
      ...
      <button onClick={() => toggle(field)} className={`w-12 h-6 rounded-full transition-colors relative ${prefs[field] ? 'bg-blue-500' : 'bg-gray-300'}`}>
  ...
  return (
    <div className="p-8">
      ...
      <div className="space-y-6">
        <ToggleItem label="Location Access" desc="Allow background location for live weather and transport routing" field="location_access" />
        <ToggleItem label="Personalized AI Recommendations" desc="Allow AI to analyze preferences for trip building" field="personalized_recommendations" />
        <ToggleItem label="Share Activity" desc="Let friends see your upcoming public trips" field="share_activity" />
        <ToggleItem label="Analytics Tracking" desc="Help us improve by sharing anonymous usage data" field="analytics_tracking" />
      </div>
    </div>
  );
};
