...
export const StaysCMS = () => {
  ...
  const columns: ColumnDef[] = [
    ...
    { key: 'price_per_night', header: 'Price', render: (_, row) => `${row.price_per_night} ${row.currency}` }
  ];
  ...
  return (
    <div className="space-y-6 p-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-navy-900">Accommodations Manager</h1>
          <p className="text-gray-500 mt-1">Manage hotels, cabins, and lodges.</p>
        </div>
        <button className="bg-navy-900 text-white px-4 py-2 rounded-lg font-bold hover:bg-navy-800 transition-colors flex items-center gap-2">
          <Plus size={18} /> Add New Stay
        </button>
      </div>

      <GenericDataTable 
        title="Accommodations Database"
        data={items}
        columns={columns}
        onEdit={(row) => {}}
      />
